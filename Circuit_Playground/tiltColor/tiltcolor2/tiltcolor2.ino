...
/*
 * This controls the half of the CP to the right and half to the left of the lowest point individually
 * using the modulo operator to find the index of the requisite light to be turned on
 * The reason that the numbers are split onto a 1-12 scale is because each position on the CP
 * has a place in an array of neoPixels, including both charging poirts, even though those ports
 * don't have lights. That is also why the placement uses those structures as indices in the 
 ...
 */
  ...
  for (int i = 11; i>6; i-=1)
  {
    CircuitPlayground.setPixelColor(pixels[((nearest + i) % 12)].index, CircuitPlayground.colorWheel(color/2 + ((i-7) * fade)));
  }
}
